...
long npix2nside_(long npix)
  {
  long res = (long)floor(sqrt(npix/12.)+0.5);
  return (res*res*12==npix) ? res : -1;
  }

long nside2npix_(const long nside)
  { return 12*nside*nside; }

int ang2pix_ring(long nside, double theta, double phi)
  {
  UTIL_ASSERT((theta>=0)&&(theta<=pi),"theta out of range");
  return ang2pix_ring_z_phi (nside,cos(theta),phi);
  }

int ang2pix_nest(long nside, double theta, double phi)
  {
  UTIL_ASSERT((theta>=0)&&(theta<=pi),"theta out of range");
  return ang2pix_nest_z_phi (nside,cos(theta),phi);
}

int vec2pix_ring(long nside, const double *vec)
  {
  double vlen=sqrt(vec[0]*vec[0]+vec[1]*vec[1]+vec[2]*vec[2]);
  return ang2pix_ring_z_phi (nside,vec[2]/vlen, atan2(vec[1],vec[0]));
  }
int vec2pix_nest(long nside, const double *vec)
  {
  double vlen=sqrt(vec[0]*vec[0]+vec[1]*vec[1]+vec[2]*vec[2]);
  return ang2pix_nest_z_phi (nside,vec[2]/vlen, atan2(vec[1],vec[0]));
  }
...
void neighbours_nest (long nside, long ipix, long *result)
  {
  int ix, iy, face_num;
  long nsm1 = nside-1;
  int order = ((nside)&(nside-1)) ? -1 : ilog2(nside);
  ...
  if ((ix>0)&&(ix<nsm1)&&(iy>0)&&(iy<nsm1))
    ...
    for (int i=0; i<8; ++i)
      {
      int x=ix+nb_xoffset[i], y=iy+nb_yoffset[i], temp;
      int nbnum=4;
      if (x<0)
        { x+=nside; nbnum-=1; }
      else if (x>=nside)
...
void neighbours_ring (long nside, long ipix, long *result)
  {
  int ix, iy, face_num;
  long nsm1 = nside-1;

  ring2xyf(nside, ipix, &ix, &iy, &face_num);

  if ((ix>0)&&(ix<nsm1)&&(iy>0)&&(iy<nsm1))
    {
      for (int m=0; m<8; ++m)
        result[m] = xyf2ring(nside, ix+nb_xoffset[m],iy+nb_yoffset[m],face_num);
    }
  else
    {
    for (int i=0; i<8; ++i)
